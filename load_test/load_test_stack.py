...
class LoadTestStack(core.Stack):
    ...
    def get_context(self):
        ...
        self.instancetype = ec2.InstanceType(
            self.node.try_get_context("instancetype"))
        ...
        self.locust_user_number = int(
            self.node.try_get_context("locust_user_number"))
        self.locust_hatch_rate = int(
            self.node.try_get_context("locust_hatch_rate"))
    ...
    def get_userdata(self, is_master):
        ...
        userdata.add_commands("""
            ...
            """ % (self.locust_version if self.locust_version else "",
                   self.asset_bucket.bucket_name))
        ...
        if is_master:
            ...
            if not self.headless:
                ...
                run_command = "locust --web-port 80 --locustfile locustfile.py %s" % (
                    "--master" if self.clustersize > 1 else "")
            # without web UI
            else:
                if self.clustersize > 1:
                    run_command = "locust --headless --users %s --spawn-rate %s --locustfile locustfile.py --expect-workers %s" % (
                        self.locust_user_number, self.locust_hatch_rate,
                        self.clustersize - 1)
                else:
                    run_command = "locust --headless --users %s --spawn-rate %s --locustfile locustfile.py" % (
                        self.locust_user_number, self.locust_hatch_rate)
        # user data for worker
        else:
            # with web UI
            if not self.headless:
                run_command = "locust --locustfile locustfile.py --worker --master-host %s" % (
                    self.master.instance_private_ip)
            # without web UI
            else:
                run_command = "locust --locustfile locustfile.py --worker --master-host %s" % (
                    self.master.instance_private_ip)
    ...
    def create_ec2_cluster(self):
        # create a new VPC
        vpc = ec2.Vpc(self, "TheVPC", cidr=self.vpc_cidr)
        ...
        master_sg = ec2.SecurityGroup(self,
                                      "MasterSecurityGroup",
                                      vpc=vpc,
                                      allow_all_outbound=True)
        master_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp(80),
                                   "allow locust port")
        master_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp(22),
                                   "allow ssh")

        # create master node
        self.master = ec2.Instance(
            self,
            "Master",
        ...
        if self.clustersize > 1:
            ...
            worker_sg = ec2.SecurityGroup(self,
                                          "workerSecurityGroup",
                                          vpc=vpc,
                                          allow_all_outbound=True)
            worker_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp(22),
                                       "allow ssh")
            master_sg.add_ingress_rule(worker_sg, ec2.Port.tcp(5557),
    ...
    def create_vpc_peering(self, vpc):
        # get peering vpc
        peering_vpc = ec2.Vpc.from_lookup(self,
                                          "PeeringVPC",
                                          vpc_id=self.vpc_to_peer)
        print(peering_vpc.public_subnets)
        # create vpc peering
        peering = ec2.CfnVPCPeeringConnection(self,
                                              "Peering",
        ...
        self.add_peering_route(vpc,
                               peering_vpc,
                               vpc_peering_id,
                               destination_cidr=self.vpc_to_peer_cidr)
